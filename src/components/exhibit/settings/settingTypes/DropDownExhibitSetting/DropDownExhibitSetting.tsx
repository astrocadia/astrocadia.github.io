...
interface DropDownExhibitSettingProps {
  setting: DropDownSetting;
  updatedValue?: DropDownSetting['value'];
  onChange: (
    id: number,
    manifestId: string,
    value: DropDownSetting['value'],
    reset: boolean
  ) => void;
  disabled?: boolean;
}
...
export const DropDownExhibitSetting: FunctionComponent<
  DropDownExhibitSettingProps
> = ({ setting, updatedValue, onChange, disabled }) => {
  ...
  return (
    <MainPanelCoplanarSubpageBlockRow className='DropDownExhibitSetting'>
      <TextField
        label={
          <SettingLabelWrapper
